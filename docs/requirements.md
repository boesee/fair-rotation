...
| FR-22 | Der Trainer kann für ein Turnier die Spieler aus dem aktiven Kader markieren, die NICHT teilnehmen (Standardannahme: der ganze aktive Kader ist dabei, da für die Turniermodi 3vs3/6vs6 ohnehin nur eine Teilmenge sinnvoll ist und Abwesenheiten oft schon Tage vorher bekannt sind). Je markiertem Spieler wird ein Grund erfasst (`privat verhindert` oder `Kader war voll`, siehe FR-44). Die Anwesenheit gilt turnierweit für alle Spiele dieses Turniers (keine erneute Erfassung pro Spiel nötig). | Ziele: Anwesenheit erfassen |
...
| FR-27 | Der Trainer kann die Turnier-Anwesenheit über die reguläre Anwesenheits-Seite korrigieren, solange das Turnier nicht vollständig `beendet` ist (FR-29) – danach nur noch über den Admin-Bereich (FR-43). Die Feldzuteilung eines einzelnen Spiels kann korrigiert werden, solange noch keine Einsätze (Ein-/Auswechslungen) für dieses Spiel erfasst wurden. | Ziele: Anwesenheit erfassen |
...
| FR-37 | Der Trainer kann mehrere Spieler (beliebige Mischung aus aktiv und Bank, über ein oder mehrere Felder hinweg) gleichzeitig auswählen und mit einem Tap alle gemeinsam wechseln – Bank-Spieler werden ein-, aktive Spieler ausgewechselt. Ersetzt wiederholtes einzelnes Ein-/Auswechseln bei Block-Rotationen (z.B. 3er-Gruppen). | Constraints: Bedienung unter Zeitdruck (NFR-1) |
| FR-45 | Der Trainer kann eine aktuelle Mehrfachauswahl von Spielern als benannten, turnierweiten "Rotationsblock" speichern (z.B. feste Wechselgruppen, die über mehrere Spiele hinweg gleich bleiben). Gespeicherte Blöcke werden als Kurzwahl-Chips angezeigt und wählen bei einem Tap alle ihre Mitglieder gleichzeitig aus (statt sie erneut einzeln anzuhaken); ein Block ist unabhängig vom Spielmodus (3vs3/6vs6) nutzbar, da die tatsächliche Feldzuteilung ohnehin pro Spiel bereits feststeht. Blöcke können gelöscht werden. | Constraints: Bedienung unter Zeitdruck (NFR-1) |
...
| FR-44 | Der Trainer kann turnierübergreifend für jeden Spieler einsehen, bei wie vielen (nicht als Test markierten) Turnieren er mit Grund "Kader war voll" nicht teilgenommen hat. Rein privat bedingte Abwesenheiten zählen bewusst nicht mit. Diese Kennzahl wird – anders als FR-41/42 – unabhängig vom Turnier-Status ermittelt (auch `geplante`/`laufende` Turniere zählen), da die Kader-Auswahl eine Rosterentscheidung ist, kein Spielergebnis, und dem Trainer schon vor bzw. während eines Turniers zur fairen Auswahl für künftige Turniere dienen soll. | Ziele: Teilnahme-Statistik, faire Kader-Rotation |

### Administration & Korrekturen
