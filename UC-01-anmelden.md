...
- Ein Benutzerkonto (Benutzername/Passwort) existiert bereits in Supabase
  Auth. Die Erstellung dieses Kontos ist kein Teil dieses Use Case, da es
  sich um ein einmaliges Setup durch den Trainer selbst handelt (kein
  Self-Service-Registrierungs-Flow in der App, vgl. Vision: ein Trainer,
  ein Konto).
  *Technischer Hinweis: Supabase Auth unterstützt nativ nur E-Mail-basierte
  Anmeldung. "Benutzername" ist ein reines UI-Label; intern wird beim
  einmaligen Setup ein interner Pseudo-E-Mail-Wert hinterlegt (z.B.
  `trainer@fair-rotation.local`). Es gibt daher keine eigene
  Benutzer-Entität im Entity-Modell – der Benutzer bleibt vollständig
  Auth-Infrastruktur (`auth.users`), kein Fachdatum.*
...
4. Der Trainer gibt Benutzername und Passwort ein und bestätigt.
5. Die App sendet die Anmeldedaten an Supabase Auth (intern als
   E-Mail/Passwort-Request, siehe technischer Hinweis oben).
...
oder unbekannter Benutzername).
1. Die App zeigt eine allgemeine Fehlermeldung ("Benutzername oder
   Passwort falsch") – ohne zu verraten, ob das Konto existiert.
...
Bei Schritt 4: Benutzername oder Passwort sind leer.
...
|---|---|---|
| T1 | Korrekter Benutzername + korrektes Passwort eingeben | Anmeldung erfolgreich, Hauptübersicht wird angezeigt (Basic Flow) |
| T2 | Falsches Passwort eingeben | Fehlermeldung, Login-Formular bleibt sichtbar (E1) |
| T3 | Unbekannten Benutzernamen eingeben | Gleiche generische Fehlermeldung wie T2 (E1) – kein Hinweis, ob das Konto existiert |
